...
if mc_current == mc_switch
	ready = true
end

if File.directory?("#{appdata}\\.minecraft") and not ready  # if there is a minecraft directory in appdata
...
if archive and not ready  # check if there is archive
    system("7z x .minecraft(#{mc_switch}).7z -o\"#{appdata}\"")  # extract the archive to appdata
end

if ready or archive or (!archive && mc_switch == "Vanilla")  # if there is archive or choice was Vanilla
